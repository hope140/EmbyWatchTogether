...
当前项目版本为 `1.6.0.5`，对应当前 beta 修订版 tag `v1.6.0.5`；它以 stable `v1.5.0.0` 为兼容基线，新增可选 GitHub Token 配置，用于认证 beta Releases API 查询，并整理了管理页设置入口。已配置 Token 时仅 API 查询带认证，stable 和固定资产下载保持原路径。`v1.6.0.4` 修正较慢 GitHub/CDN 网络下固定资产下载超时过短的问题；`v1.6.0.3`、`v1.6.0.2` 及更早版本保留为历史 beta。`v1.5.0.4` 仅保留为失败的发布尝试和 Git tag，因 CI flaky test 在 Release 创建前中止，没有 GitHub Release 或发布资产，不得复用该 tag。已发布的 beta 历史版本为 `v1.5.0.1`、`v1.5.0.2`、`v1.5.0.3`、`v1.6.0.1`、`v1.6.0.2`、`v1.6.0.3` 和 `v1.6.0.4`。Phase D 实机验收通过后的暂定目标 stable 为 `1.6.1.0` / `v1.6.1.0`，不属于当前版本。一个完整的 beta 到 stable 示例是 `1.4.0.0` -> `1.4.0.1`（beta）-> `1.4.1.0`（stable）；正式版必须至少改变 MAJOR、MINOR、PATCH 之一。`1.3.0.6` 及更早的 `1.3.0.x` 保留为历史版本，不因本次修正而覆盖或重命名；历史版本整理不移动、重命名或重建已有 tag。插件默认选择 `stable`，管理员可在插件配置页选择 `beta`，更新任务按所选通道获取并自动安装；`stable` 仍只读取 `releases/latest`。
...
- 上一兼容性修复测试版：`1.4.3.2` / `v1.4.3.2`；
- 当前兼容性修复正式版：`1.4.4.0` / `v1.4.4.0`；
- 上一诊断能力测试版：`1.4.4.1` / `v1.4.4.1`；
- 上一参与者重新同步测试版：`1.4.4.2` / `v1.4.4.2`；
- 上一非管理员入口修复测试版：`1.4.4.3` / `v1.4.4.3`；
- 上一非管理员页面布局优化测试版：`1.4.4.4` / `v1.4.4.4`；
- 上一诊断展示持久化测试版：`1.4.4.6` / `v1.4.4.6`；
- 上一 S1—S4 稳定版：`1.4.5.0` / `v1.4.5.0`；
- 上一阶段 C 测试版：`1.4.5.1` / `v1.4.5.1`；
- 上一阶段 C 修订版：`1.4.5.2` / `v1.4.5.2`；
- 当前阶段 C 稳定版：`1.5.0.0` / `v1.5.0.0`；
- 已发布 beta：`1.5.0.1` / `v1.5.0.1`；
...
- 上一 beta 修订版：`1.6.0.3` / `v1.6.0.3`；
- 上一 beta 修订版：`1.6.0.4` / `v1.6.0.4`；
- 当前 beta 修订版：`1.6.0.5` / `v1.6.0.5`；
