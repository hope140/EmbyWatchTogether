...
Watch Together 是一个运行在 Emby Server 内的双人同步观看插件，当前项目版本为 `1.3.0.5`。它读取同一台服务器上的会话状态，并通过 Emby 远程控制协调起播、暂停/继续、手动拖动进度、切换视频和停止播放。
...
- 一方持续确认停止后，默认暂停另一方并发送提示；短暂断线、旧会话或合法的拖到 0 不会立即触发停止处理，暂停和提示行为可以分别关闭。
