...
例如，`1.2.0.14` 与 `v1.2.0.14` 是一对匹配的版本值；`1.02.0.14`、`v1.2.0.14-beta` 和项目版本为 `1.2.0.14` 但 tag 为 `v1.2.0.15` 均无效。
...
- `MINOR` 递增：`1.3.0.0`；
- `PATCH` 递增：`1.2.1.0`；
- `REVISION` 递增：`1.2.0.14`。

不要为了保持版本连续而修改已经发布的版本号，也不要复用已经存在的 tag。

## 当前版本和后续示例

当前项目版本为 `1.2.0.13`，对应正式 tag 为 `v1.2.0.13`。现有 `1.2.0.x` 历史版本主要使用第四段递增；该历史约定保留，但从后续发布开始按本文区分功能、修复和修订级别。

以当前版本为基准：

- 小范围兼容性修正：`1.2.0.14` / `v1.2.0.14`；
- 一组明确的用户可感知修复：`1.2.1.0` / `v1.2.1.0`；
- 新增兼容功能：`1.3.0.0` / `v1.3.0.0`；
...
4. 运行构建、测试和发布校验；
5. 手动发布 workflow 时，将同一个 tag 作为 `tag` 输入。
