...
Watch Together 是一个运行在 Emby Server 内的双人同步观看插件，当前 beta 版本为 `1.6.0.1`，稳定版为 `1.5.0.0`。它读取同一台服务器上的会话状态，并通过 Emby 远程控制协调起播、暂停/继续、手动拖动进度、切换视频和停止播放。
...
- 主用户切换到下一集或其他视频时，参与者会先自动打开主用户当前视频，确认会话实际进入相同 Item 后再通过 Barrier 对齐；参与者自行换片不会让主用户跟随。
- 已经进入 `Watching` 的房间遇到短暂 SessionInfo 缺失时会显示“临时掉线恢复中”，最多等待 10 秒；只有原 User、原 SessionId 和原 ItemId 恢复后才重新对齐，不会自动迁移到新设备。
- 播放过程中会同步明确的暂停/继续和明显的手动拖动，不会为了消除小幅播放差异而持续跳转。
- 长时间播放中的明显持续 drift 会被低频观测；达到条件时最多触发一次保守 Barrier 纠偏，自动纠偏之间保持冷却。
...
- 诊断页面还会展示临时掉线恢复的预期身份摘要、剩余窗口、当前/最大 drift、最近自动纠偏和纠偏次数。
...
- 同步基于服务器提供的会话状态，不是播放器内部时钟；不保证每一帧完全相同，小幅漂移会保留，长期明显偏移时才会低频触发一次保守纠偏。
