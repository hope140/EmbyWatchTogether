...
当前项目版本为 `1.4.3.0`，对应正式版发布 tag `v1.4.3.0`。本版本在 `1.4.2.2` beta 验收后递增 PATCH，属于 stable 正式版，不复用任何既有 tag。一个完整的 beta 到 stable 示例是 `1.4.0.0` -> `1.4.0.1`（beta）-> `1.4.1.0`（stable）；本次 `1.4.1.0` -> `1.4.2.0` 也属于正式版，因为前三段中的 PATCH 发生了变化。beta 可以只递增第四段，正式版必须至少改变 MAJOR、MINOR、PATCH 之一。`1.3.0.6` 及更早的 `1.3.0.x` 保留为历史版本，不因本次修正而覆盖或重命名；历史版本整理不移动 tag。后续 beta 从 `beta` 分支发布为 GitHub prerelease，插件默认选择 `stable`，管理员可在插件配置页选择 `beta`，更新任务按所选通道获取并自动安装；`stable` 仍只读取 `releases/latest`。
...
- 上一兼容性修复正式版：`1.4.2.0` / `v1.4.2.0`；
- 上一兼容性修复测试版：`1.4.2.2` / `v1.4.2.2`；
- 当前兼容性修复正式版：`1.4.3.0` / `v1.4.3.0`；
