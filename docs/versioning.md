...
当前项目版本为 `1.3.0.4`，对应发布 tag 预期为 `v1.3.0.4`。`1.3.0.3`、`1.3.0.2` 和 `1.3.0.1` 保留为已发布历史版本，不因本次修正而覆盖或重命名；本次 beta 尚未创建正式 Release。现有 `1.2.0.x` 历史版本主要使用第四段递增；该历史约定保留，后续发布按本文区分功能、修复和修订级别。

以当前版本为基准：

- 小范围兼容性修正：`1.3.0.4` / `v1.3.0.4`；
