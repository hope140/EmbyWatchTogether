...
- `PlaybackStopped` 事件只用于立即唤醒轮询，不直接确认停止。只有 `SessionSelector` 选中的当前会话持续显示停止、离线或缺失达到 2 秒后才处理；旧 stopped 会话不会覆盖当前播放，恢复有效快照会取消本次判定，合法的 seek-to-zero 不单独视为停止。
...
递增高位时，右侧各段归零，例如 `MAJOR` 递增为 `2.0.0.0`，`MINOR` 递增为 `1.3.0.0`，`PATCH` 递增为 `1.2.1.0`，`REVISION` 递增为 `1.2.0.15`。项目文件中的 `Version`、`FileVersion`、`AssemblyVersion` 必须完全一致且不带 `v`；Git tag 使用 `v` 前缀并与三项版本一致，例如 `1.2.0.15` 对应 `v1.2.0.15`。当前项目版本为 `1.3.0.4`，对应发布 tag 预期为 `v1.3.0.4`；本次仍处于 beta 手动部署观察阶段，正式 Release 尚未创建。
