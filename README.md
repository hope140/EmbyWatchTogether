...
Watch Together 是一个运行在 Emby Server 内的双人同步观看插件，当前项目版本为 `1.3.0.0`。它读取同一台服务器上的会话快照，并通过 Emby 远程控制命令协调起播、暂停/继续、用户手动拖动进度、切换视频和停止播放。
...
- Emby 的 `PlaybackStopped` 事件只唤醒轮询，不直接确认停止；只有 SessionSelector 选中的当前会话持续显示停止、离线或缺失达到 2 秒后才处理，同一用户的旧 stopped 会话不会覆盖当前播放。期间恢复有效快照会取消本次判定；合法的 seek-to-zero 不会单独被当成停止。确认停止后默认暂停另一方并发送提示消息，两个行为可以分别关闭。
