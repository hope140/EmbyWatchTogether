...
Watch Together 是一个运行在 Emby Server 内的双人同步观看插件，当前 beta 版本为 `1.6.0.4`，稳定版为 `1.5.0.0`。它读取同一台服务器上的会话状态，并通过 Emby 远程控制协调起播、暂停/继续、手动拖动进度、切换视频和停止播放。
...
- 主用户切换到下一集或其他视频时，参与者会先自动打开主用户当前视频，确认会话实际进入相同 Item 后再通过 Barrier 对齐；主用户的新会话晚于停止观察窗口出现时，插件会在最长 10 秒的有界恢复窗口内继续识别这次换片；参与者自行换片不会让主用户跟随。
- 播放过程中会同步明确的暂停/继续和明显的手动拖动，不会为了消除小幅播放差异而持续跳转。
- 已经进入 `Watching` 且会话身份未变化时，单端远控标志短暂丢失会等待最多 8 秒恢复；窗口内不发送新的同步命令，恢复后继续观察，持续异常仍按安全规则暂停并重新同步。
- 普通用户可以创建 15 分钟有效的一次性邀请码；另一位已登录用户接受后才创建双人房间，邀请码不写入持久化文件，服务器重启后失效。
...
- 已加入房间的普通参与者可以从用户菜单进入页面，并请求重新同步；创建、删除和管理员控制仍受管理员权限保护。
- 自助房间的创建者可以结束自己的房间；管理员仍可结束所有房间。
- 任一参与者都可以结束自己所在的房间；“暂离房间”只退出同步状态，不解除成员关系。
- 诊断页面展示状态原因、会话能力、Pending、Barrier、恢复窗口和最近事件，并支持脱敏 JSON 导出。
...
- 初始同步时两个会话必须打开相同视频、媒体时长相差不超过 3 秒、播放速率接近正常速度，并同时支持暂停、继续和拖动进度等远程控制。Watching 中主用户换片会经过有限的 Handoff；客户端没有所需能力时房间会停留在 `Waiting`。
...
插件更新由 Emby 计划任务处理，插件配置页提供更新通道选择和 beta GitHub API Token 配置，但不提供独立的检查或安装按钮。服务器启动后，**Dashboard → 计划任务** 中会出现名为“Watch Together 更新检查”的任务，默认每 24 小时运行一次；管理员可以在那里调整检测时间、禁用任务或手动执行。任务会按插件配置中的 `stable`/`beta` 通道检查并自动安装。

`stable` 使用固定的官方 `releases/latest` 资产；`beta` 使用 GitHub Releases API 在非 draft stable 与 prerelease 中选择最高规范版本，然后只校验该版本的固定资产并构造对应 tag 的官方资产。历史低版本缺少固定资产时不会阻断最高版本选择；最高版本自身缺少资产仍会拒绝更新。两种通道都会在安装前校验发布签名、文件大小、哈希、程序集名称和版本；校验失败时不会安装。安装由 Emby 的插件安装器负责，插件不会自行覆盖 DLL，也不会调用重启或关机。安装成功后插件会通知 Emby“等待重启”，仪表盘会出现重启提示；如果检查后已是最新版本，当前管理员会话也会收到同样的短横条提示；重启前同一版本不会重复安装。
...
1. 登录 Emby 后打开 **Watch Together** 页面，在“邀请一起观看”区域填写可选房间名称并创建邀请码。
2. 立即复制邀请码，发送给另一位已登录同一服务器的用户；邀请码有效期为 15 分钟且只能接受一次。
3. 对方输入邀请码并接受后，双方会看到同一个自助房间，再分别打开同一视频。
4. 邀请创建者可以撤销未接受的邀请码，也可以在房间卡片上结束已经创建的自助房间。

邀请码只在创建响应和当前页面中显示，不会写入 `rooms.json`；Emby 重启后未接受的邀请码会失效，需要重新创建。
