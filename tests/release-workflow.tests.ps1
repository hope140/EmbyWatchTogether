...
Assert-Matches -Text $workflowText -Pattern '(?m)^\s{10}GH_TOKEN:\s*\$\{\{\s*github\.token\s*\}\}\s*$' -Message 'The release validation step must map GH_TOKEN from github.token.'
...
Assert-Matches -Text $runText -Pattern '\$canonicalVersionPattern\s*=\s*''\\Av\(\?:0\|\[1-9\]\[0-9\]\*\)\\\.\(\?:0\|\[1-9\]\[0-9\]\*\)\\\.\(\?:0\|\[1-9\]\[0-9\]\*\)\\\.\(\?:0\|\[1-9\]\[0-9\]\*\)\\z''' -Message 'The workflow must require exactly four numeric tag parts.'
Assert-NotMatches -Text $runText -Pattern 'v<3 or 4 numeric-part>' -Message 'The workflow must not retain the three-part tag rule.'
Assert-Matches -Text $runText -Pattern 'GH_TOKEN|gh\s+api\s+"repos/\$env:GITHUB_REPOSITORY/releases\?per_page=100"' -Message 'The stable validation must use the GitHub Releases API.'
Assert-Matches -Text $runText -Pattern 'select\(\.draft\s*==\s*false\s+and\s+\.prerelease\s*==\s*false\)' -Message 'The stable validation must exclude draft and prerelease Releases.'
Assert-Matches -Text $runText -Pattern 'Sort-Object\s+-Descending|highestFormalVersion' -Message 'The stable validation must find the highest formal Release version.'
Assert-Matches -Text $runText -Pattern '\$tagVersion\s+-le\s*\$highestFormalVersion' -Message 'The stable version must be higher than the highest formal Release.'
Assert-Matches -Text $runText -Pattern '(?s)\$tagVersion\.Major.*?\$highestFormalVersion\.Major.*?\$tagVersion\.Minor.*?\$highestFormalVersion\.Minor.*?\$tagVersion\.Build.*?\$highestFormalVersion\.Build' -Message 'The stable validation must require a MAJOR, MINOR, or PATCH change.'
Assert-Matches -Text $runText -Pattern 'REVISION-only increments are prerelease-only' -Message 'The workflow must fail closed on stable REVISION-only increments.'
Assert-Matches -Text $runText -Pattern 'if\s*\(\$historicalFormalVersions\.Count\s+-gt\s+0\)' -Message 'The stable validation must allow the first formal Release when no history exists.'
