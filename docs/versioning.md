...
当前项目版本为 `1.6.0.1`，对应当前 Phase D beta 候选 tag `v1.6.0.1`；它以 stable `v1.5.0.0` 为兼容基线，包含此前 `v1.5.0.3` beta 修复并增加 Media Handoff。`v1.5.0.4` 仅保留为失败的发布尝试和 Git tag，因 CI flaky test 在 Release 创建前中止，没有 GitHub Release 或发布资产，不得复用该 tag。已发布的 beta 历史版本为 `v1.5.0.1`、`v1.5.0.2` 和 `v1.5.0.3`。Phase D 实机验收通过后的暂定目标 stable 为 `1.6.1.0` / `v1.6.1.0`，不属于当前版本。一个完整的 beta 到 stable 示例是 `1.4.0.0` -> `1.4.0.1`（beta）-> `1.4.1.0`（stable）；正式版必须至少改变 MAJOR、MINOR、PATCH 之一。`1.3.0.6` 及更早的 `1.3.0.x` 保留为历史版本，不因本次修正而覆盖或重命名；历史版本整理不移动、重命名或重建已有 tag。插件默认选择 `stable`，管理员可在插件配置页选择 `beta`，更新任务按所选通道获取并自动安装；`stable` 仍只读取 `releases/latest`。
...
- 当前 Phase D beta：`1.6.0.1` / `v1.6.0.1`；
