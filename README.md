...
Watch Together 是一个运行在 Emby Server 内的双人同步观看插件，当前项目版本为 `1.4.0.0`。它读取同一台服务器上的会话状态，并通过 Emby 远程控制协调起播、暂停/继续、手动拖动进度、切换视频和停止播放。
...
`stable` 使用固定的官方 `releases/latest` 资产；`beta` 使用 GitHub Releases API 选择 prerelease 后构造对应 tag 的官方资产。两种通道都会在安装前校验发布签名、文件大小、哈希、程序集名称和版本；校验失败时不会安装。安装由 Emby 的插件安装器负责，插件不会自行覆盖 DLL，也不会调用重启或关机。安装成功后插件会通知 Emby“等待重启”，仪表盘会出现重启提示；重启前同一版本不会重复安装。
