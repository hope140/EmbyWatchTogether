...
当前项目版本为 `1.4.4.5`，对应 beta 预发布 tag `v1.4.4.5`；它以 `1.4.4.0` stable 为兼容基线，仅递增第四段，内容包含 S1—S4 的回归、诊断、页面和参与者重新同步能力。该版本从 `beta` 分支发布为 GitHub prerelease，不代表真实双端播放验收已经完成。一个完整的 beta 到 stable 示例是 `1.4.0.0` -> `1.4.0.1`（beta）-> `1.4.1.0`（stable）；正式版必须至少改变 MAJOR、MINOR、PATCH 之一。`1.3.0.6` 及更早的 `1.3.0.x` 保留为历史版本，不因本次修正而覆盖或重命名；历史版本整理不移动、重命名或重建已有 tag。插件默认选择 `stable`，管理员可在插件配置页选择 `beta`，更新任务按所选通道获取并自动安装；`stable` 仍只读取 `releases/latest`。
